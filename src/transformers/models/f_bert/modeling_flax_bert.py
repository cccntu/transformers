...
class FlaxFBertFourierTransform(nn.Module):
    """
    Fourier Transform layer. Applies 2D Fourier transform over final two dimensions of inputs - typically the sequence
    and hidden dimensions.
    ...
    def setup(self):
        if not self.config.use_tpu_fourier_optimizations:
            ...
            dft_mat_hidden = linalg.dft(self.config.hidden_size)
            dft_mat_seq = linalg.dft(self.config.max_position_embeddings)
        ...
        elif not math.log2(self.config.max_position_embeddings).is_integer():
            raise ValueError(
                ...
                "dimension. config.max_seq_length: %d. config.d_model: $d" % self.config.max_position_embeddings,
                self.config.d_model,
            )
        else:
...
class FlaxBertAttention(nn.Module):
    ...
    def setup(self):
        self.self = FlaxFBertFourierTransform(self.config, dtype=self.dtype)
